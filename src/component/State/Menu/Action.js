...
export const getMenuItemsByRestaurantId = (reqData) => {
    return async (dispatch) => {
        ...
        try {
            const { data } = await api.get(`/api/food/restaurant/${reqData.restaurantId}?vegeterian=${reqData.vegetarian}&nonveg=${reqData.nonveg}
...
export const updateMenuItemsAvailability = ({ foodId, jwt }) => {
    return async (dispatch) => {
        ...
        try {
            const { data } = await api.get(`/api/admin/food/${foodId}`, {},
                {
                    headers: {
                        Authorization: `Bearer ${jwt}`
                    }
